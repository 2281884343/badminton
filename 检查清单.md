# ✅ 部署前检查清单

在部署到生产环境之前，请确保完成以下检查：

## 🔐 安全检查

- [ ] 已修改API密钥（不要使用示例中的密钥）
  - 编辑 `backend/.env` 或 `backend/main.py`
  - 获取你自己的Moonshot API密钥

- [ ] 已配置防火墙规则
  ```bash
  sudo ufw enable
  sudo ufw allow 80/tcp
  sudo ufw allow 443/tcp
  sudo ufw allow 22/tcp
  ```

- [ ] 已设置SSL证书（如果使用HTTPS）
  ```bash
  sudo certbot --nginx -d your-domain.com
  ```

- [ ] 已修改Nginx配置中的域名/IP
  - 编辑 `nginx.conf`
  - 修改 `server_name` 为你的域名

- [ ] 已检查文件权限
  ```bash
  chmod 755 deploy.sh start-dev.sh start-prod.sh
  chmod 644 badminton-game.service nginx.conf
  ```

## 🚀 环境检查

- [ ] Python版本 ≥ 3.8
  ```bash
  python3 --version
  ```

- [ ] Node.js版本 ≥ 16
  ```bash
  node --version
  ```

- [ ] 已安装必要的系统包
  ```bash
  # Ubuntu/Debian
  sudo apt install python3-venv nginx
  ```

- [ ] 磁盘空间充足（建议 ≥ 2GB可用空间）
  ```bash
  df -h
  ```

- [ ] 内存充足（建议 ≥ 1GB可用内存）
  ```bash
  free -h
  ```

## 📁 文件配置检查

- [ ] 已修改 `badminton-game.service` 中的路径
  - `WorkingDirectory=/path/to/bm/backend`
  - `Environment="PATH=/path/to/bm/backend/venv/bin"`
  - `ExecStart=/path/to/bm/backend/venv/bin/python main.py`
  - `User=your-username`

- [ ] 已修改 `nginx.conf` 中的路径
  - `server_name your-domain.com`
  - `root /path/to/bm/frontend/dist`

- [ ] 创建了数据目录
  ```bash
  mkdir -p backend/data/players
  ```

## 🔨 构建和依赖

- [ ] 后端依赖已安装
  ```bash
  cd backend
  source venv/bin/activate
  pip list
  ```

- [ ] 前端依赖已安装
  ```bash
  cd frontend
  ls node_modules/ | wc -l  # 应该有很多文件
  ```

- [ ] 前端已构建（生产环境）
  ```bash
  cd frontend
  ls -la dist/  # 应该有index.html和assets/
  ```

## 🌐 网络检查

- [ ] 后端端口8000可访问
  ```bash
  curl http://localhost:8000/api/skills
  ```

- [ ] 前端端口3000可访问（开发模式）
  ```bash
  curl http://localhost:3000
  ```

- [ ] WebSocket连接正常
  ```bash
  # 在浏览器控制台测试
  # ws = new WebSocket("ws://localhost:8000/ws/test/testuser")
  ```

- [ ] API密钥有效
  ```bash
  curl -X POST https://api.moonshot.cn/v1/chat/completions \
    -H "Authorization: Bearer YOUR_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{"model": "kimi-k2-turbo-preview", "messages": [{"role": "user", "content": "test"}]}'
  ```

## 🔄 服务状态检查

- [ ] Nginx服务正常
  ```bash
  sudo systemctl status nginx
  sudo nginx -t
  ```

- [ ] 后端服务正常
  ```bash
  sudo systemctl status badminton-game
  ```

- [ ] 服务自动启动已启用
  ```bash
  sudo systemctl is-enabled badminton-game
  sudo systemctl is-enabled nginx
  ```

## 📊 监控和日志

- [ ] 能够查看后端日志
  ```bash
  sudo journalctl -u badminton-game -f
  ```

- [ ] 能够查看Nginx日志
  ```bash
  sudo tail -f /var/log/nginx/access.log
  sudo tail -f /var/log/nginx/error.log
  ```

- [ ] 已设置日志轮转（可选）
  ```bash
  ls /etc/logrotate.d/nginx
  ```

## 💾 备份检查

- [ ] 已设置数据备份计划
  ```bash
  crontab -l  # 查看是否有备份任务
  ```

- [ ] 测试备份恢复流程
  ```bash
  cp -r backend/data/players /tmp/test-backup
  ```

## 🧪 功能测试

- [ ] 创建测试用户成功
- [ ] 保存技能配置成功
- [ ] 创建房间成功
- [ ] 加入房间成功
- [ ] WebSocket连接成功
- [ ] 发送击球动作成功
- [ ] AI描述生成成功
- [ ] 比分计算正确
- [ ] 游戏结束判定正确

## 🎯 性能测试

- [ ] 单用户响应时间 < 1秒
- [ ] 多用户同时在线测试（建议测试10+用户）
- [ ] 长时间运行稳定性测试（建议运行24小时）
- [ ] 内存使用正常（不持续增长）
- [ ] CPU使用率正常（< 80%）

## 📱 浏览器兼容性

- [ ] Chrome浏览器测试通过
- [ ] Firefox浏览器测试通过
- [ ] Safari浏览器测试通过（如果有Mac）
- [ ] 移动端浏览器测试通过

## 🔧 故障恢复

- [ ] 知道如何重启服务
  ```bash
  sudo systemctl restart badminton-game
  sudo systemctl restart nginx
  ```

- [ ] 知道如何查看日志
  ```bash
  sudo journalctl -u badminton-game -n 100
  ```

- [ ] 知道如何回滚更新
  ```bash
  git checkout <previous-commit>
  ./deploy.sh
  ```

- [ ] 有紧急联系方式和备用方案

## 📝 文档检查

- [ ] 已记录服务器配置信息
  - 服务器IP/域名
  - SSH登录信息
  - 各服务端口
  - API密钥保存位置

- [ ] 已创建操作手册给其他管理员

- [ ] 已记录常见问题和解决方案

## 最后确认

- [ ] 所有必需的检查项都已完成 ✓
- [ ] 已在测试环境验证过 ✓
- [ ] 已准备好监控和日志工具 ✓
- [ ] 已通知相关人员部署计划 ✓
- [ ] 已准备好回滚方案 ✓

---

**检查完成后即可部署！🚀**

部署命令：
```bash
./deploy.sh
./start-prod.sh
# 或
sudo systemctl start badminton-game
```

祝部署顺利！如遇问题请查看 [部署指南-Linux.md](./部署指南-Linux.md)

